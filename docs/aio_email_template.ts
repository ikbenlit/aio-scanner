...
export function generateScanEmailTemplate(scanResults: ScanResult): string {
  ...
  const getPriorityFindings = (modules: ModuleResult[]) => {
    ...
    <div class="findings-section">
      ...
      ${topFindings.map((finding, index) => `
        <div class="finding-item finding-${finding.severity}">
          <div class="finding-icon">${finding.severity === 'high' ? '🚨' : '⚠️'}</div>
...
export function generateScanEmailText(scanResults: ScanResult): string {
  const { url, overallScore, moduleResults } = scanResults;
  const domain = new URL(url).hostname;
